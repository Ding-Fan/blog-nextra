...
In commands, we use [/command | /alias] to represent the command and its alias. The [-o | --option] represents the optional parameters or flags that can be used with the command. The **the input here** is the input that the user needs to provide to execute the command. 
```
