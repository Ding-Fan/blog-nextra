...
### summary my requirements about japanese improvement

日本語の表現をより自然でプロフェッショナルにするためのフィードバック依頼プロンプト

依頼の目的:

私の日本語を、単に文法的に正しいだけでなく、日本のビジネスやITの現場で実際に使われる、より自然で適切な表現に向上させたいです。以下の文章を添削し、改善案を提案してください。

依頼事項:

以下の文章について、文法的な間違いの修正はもちろんのこと、特に下記の点を重視して、複数の表現の選択肢と、なぜその表現が適切なのかという詳しい解説をお願いします。



自然な言い回しへの改善

教科書のような直訳的で硬い表現があれば、ネイティブスピーカーが日常的に使うような、より自然で流暢な言い回しに修正してください。

文章から伝わる感情やニュアンス（例：安心、感謝、依頼、恐縮）が、より豊かになるような表現を教えてください。

状況に応じた丁寧さ（敬語）の調整

相手（例：親しい同僚、上司、クライアント）や状況（例：Slackでのチャット、フォーマルなメール、会議）に応じて、最適な丁寧さのレベルを提案してください。

現在の表現が「丁寧すぎる」または「カジュアルすぎる」場合は、その理由と代替案を示してください。

それぞれの表現が持つ微妙なニュアンスや、相手に与える印象の違いについて解説をお願いします。

専門用語とビジネス語彙の最適化

特にITやソフトウェア開発の文脈で、より一般的で的確な専門用語があれば修正してください。（例：「エラーが出てきます」→「エラーが発生します」）

ビジネスシーンにふさわしい、より洗練された語彙を選択してください。（例：「足りない」→「不足している」）

実践的なコミュニケーションスキルの向上

依頼や質問をする際に、相手への配慮が伝わる「クッション言葉」（例：「お忙しいところ恐縮ですが」「もし可能でしたら」「お手すきの際に」）を効果的に使った表現を提案してください。

意図が明確に、かつ簡潔に伝わる文章構成にしてください。特にチャットなどスピードが求められる場面で使える、短く的確な「一行メッセージ」のような表現も知りたいです。
