...
For detailed shell setup and Fish configuration, see [Shell Setup Guide](./fedora.guide.shell.mdx).


[install bitwarden](./fedora.guide.flathub.mdx)
...
```bash
sudo dnf install -y swappy grim slurp
...
use stow to sync dotfiles/configs
sudo dnf install -y stow


### Essential System Tools
```bash
sudo dnf install -y tldr 7z htop unar fastfetch wget curl btop wavemon telnet strawberry calibre brightnessctl mako
...
[Development Tools Manager (mise)](./fedora.mise.mdx)
